...
#Default duration to 10 seconds if not set.
if($Duration -eq 0)
	{$Duration = 10}
...
else
{
	...
	$Filter = 
	"Operation,is,writeFile,include;ProcessName,is,System,Exclude"
	$FilterObj = convert-FilterToObj 
	if($FilterObj -match "Error:")
		{return $FilterObj}
}
...
$TempCSVname = ".\Temp_$FileDate.txt"
...
New-Item C:\Users\danie\Desktop\Procmon\$TempCSVname
